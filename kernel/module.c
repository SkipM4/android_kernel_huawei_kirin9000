...
struct module *find_dfx_module(const char *name, size_t len)
{
...
static void hhee_lkm_update(const struct module_layout *layout)
{
#ifdef CONFIG_HHEE
...
static void cfi_init(struct module *mod)
{
	...
	cfi_harden_module_init(mod, find_kallsyms_symbol_value(mod, "__cfi_check"));
#else
